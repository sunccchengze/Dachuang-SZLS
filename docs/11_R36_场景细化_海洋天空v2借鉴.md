...
> before/after 像素证据见 `docs/research/shots/r36/ocean_v2/`（4 机位同参对照）。
>
> **合并状态（2026-09-17，R39）**：本文件是**并行实现**（分支 `arena/01a074a3-dachuang-szls`，
> commit `595daf2`）的原始记录，已被正式并入主线（merge commit `44efb33`）。代码按
> `docs/research/round38_残项收口与裁决.md` §一 的逐项裁决处理：
> - 采纳并重写：FRAG 波高与 VERT 波系**同方向同相位**、`seaSky` 真天空反射、云与日月次序 → 见
>   `docs/research/round39_分支合并与T4落地.md`（不照抄本分支写法，必须落在主线基线上）；
> - 否决：波群 2→5 波与场心收敛 0.5→0.25（转 T5 浪形重标定专项，理由见 round38 §一）、
>   `coastSignedDist`（主线 `shoreSigned` 是含岛/海岬/海蚀柱的 SDF 并集，严格更全）、
>   夜间月光云（主线 R36 卷云定档为昼间，夜间不抢星野）；
> - 本文件中的自测数字（66/66 等）是**该分支当时**的口径，不代表合并后主线的数字。
...
## 三、像素证据（`docs/research/shots/r36/ocean_v2/`）

| 图 | 机位/时刻 | 看什么 |
|---|---|---|
| `ocean_v2/before_hero.png` / `ocean_v2/after_hero.png` | `?debug=1&t=10&cam=60,22,990` | 海面从"平玻璃"→ 中浪浪脊 + 天光反射 + 远岸泡沫带；塔基区仍平稳 |
| `ocean_v2/before_coast.png` / `ocean_v2/after_coast.png` | `?debug=1&t=10&cam=10,8,1500` | 低机位：水线不再是满幅拍滩，岸线 90m 内浅水冰青 + 滚动碎浪带；岛屿水线保留 |
| `ocean_v2/before_night.png` / `ocean_v2/after_night.png` | `?debug=1&t=22&cam=60,22,990` | 夜场：月光反射/SSS 更立体，碎浪带减淡不刺眼 |
| `ocean_v2/before_sky2.png` / `ocean_v2/after_sky2.png`（另有 `before_sky_day.png`） | `?debug=1&t=14&cam=25,10,2600` | 天空：白天从平渐变 → 克制的冰青云族（覆盖率 ~30%），日轮穿出 |
...
| `grassField.tsx` 孤儿文件 | 仍是 37 行红方块 debug（未接入 App），与本轮无关，清理项保留（【R40 校正 2026-09-17】：R38 重写、R40 挂载+剔除，孤儿状态已结束，见 docs/research/round40） |
